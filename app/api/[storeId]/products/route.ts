...
export async function POST(
  ...
  try {
    ...
    if (!images || !images.length) {
      return new NextResponse("Imágenes son requeridas", { status: 400 });
    }
    ...
    if (!categoryId) {
      return new NextResponse("Category id es Requerida", { status: 400 });
    }
    ...
    if (!sizeId) {
      return new NextResponse("Size id es Requerido", { status: 400 });
    }
